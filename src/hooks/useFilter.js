...
const useFilter = (data) => {
  ...
  const [varinats, setVarinats] = useState("");
  ...
  const serviceData = useMemo(() => {
    ...
    if (location.pathname === "/dashboard") {
      // filtering orders by status delivered pending delivered
    ...
    //products filtering
    // filter products by parent name
    if (filter) {
    ...
    if (Addon) {
      services = services.filter((search) =>
        search.title.toLowerCase().includes(Addon.toLowerCase())
      );
    }
    //addon searching
    if (varinats) {
      services = services.filter((search) =>
        search.title.toLowerCase().includes(varinats.toLowerCase())
      );
    }
    //admin Filtering
  ...
  //handle submit addon
  const handleSubmitAddon = (e) => {
    e.preventDefault();
    setAddon(AddonRef.current.value);
  };
  //handle submit variant
  const handleSubmitVariant = (e) => {
    e.preventDefault();
    setVarinats(VariantRef.current.value);
